...
function HouseGraphic(props) {
	...
	useEffect(() => {
		httpRequest
			.post("/barometer/retrieveScore", {}, { user_id })
			.then(({ score, upcomingTodos }) => {
				if (score && !upcomingTodos) {
					return console.log("missing todos")
				}
				if (score && upcomingTodos) {
					setLoading(true)
					if (score * 100 >= 85) {
	...
	return (
		<div className='housePic'>
			{loading && <img src={pic} alt='house-score' />}
			<div className='gauge'>
				<h2>House Health</h2>
				{loading && <Gauge score={Number(score)} />}
			</div>
		</div>
	)
}
