...
module.exports = {
	login: req => {
		...
		return db.users
			...
			.then(user => {
				//  ;
				if (!user) throw "No user associated with email"
	...
	register: req => {
		...
		const { firstName, lastName, email, password, phoneNum, addressLine1, addressLine2, city, state, zip } = req.body
	...
	me: req => {
		...
		else return { success: false, msg: "Not logged in" }
	},
	info: req => {
		const db = req.app.get("db")
		...
		return db.home_address.findOne({ user_id }).then(address => {
			return address
		})
	},
	change: req => {
